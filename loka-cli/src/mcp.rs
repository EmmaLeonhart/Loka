...
//! - **Server mode**: connects to a running Loka HTTP endpoint
...
const GITHUB_REPO: &str = "EmmaLeonhart/Loka";
...
fn handle_resources_list(id: &Value, ctx: &McpContext) -> Value {
    let mut resources = vec![
        json!({
            ...
            "description": "Current Loka connection details: mode, endpoint, authentication status.",
            "mimeType": "application/json"
        }),
        json!({
            "uri": "loka://version",
            "name": "Version",
            "description": "Loka version and build info.",
            "mimeType": "application/json"
        }),
    ];
...
async fn handle_resources_read(id: &Value, params: &Value, ctx: &McpContext) -> Value {
    ...
    let result = match uri {
        ...
        "loka://schema" => read_schema_resource(ctx).await,
        ...
        "result": {
            "tools": [
                ...
                },
                {
                    ...
                    "description": "Download and install Loka Studio (the GUI dashboard). Downloads the pre-built binary from GitHub releases for the current platform. Loka Studio provides graph visualization, HNSW health diagnostics, SPARQL query editor, and ontology browsing.",
                    "inputSchema": {"type": "object", "properties": {}}
                },
                {
                    "name": "launch_studio",
                    "description": "Launch Loka Studio. Opens the GUI dashboard connecting to the current Loka instance. Downloads Studio first if not already installed.",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "endpoint": {
                                "type": "string",
                                "description": "Loka endpoint URL for Studio to connect to (defaults to current MCP connection)"
                            }
                        }
                    }
                },
                {
                    "name": "check_update",
                    "description": "Check if a Loka update is available. Returns current version, latest version, and auto-update status.",
                    "inputSchema": {"type": "object", "properties": {}}
                },
                {
                    "name": "decline_update",
                    "description": "Decline the pending auto-update. By default, Loka auto-updates 2 minutes after startup if a new version is available. Call this to cancel.",
                    "inputSchema": {"type": "object", "properties": {}}
                }
            ]
        }
...
async fn startup_update_check(
    ...
    // Also update Loka Studio if it is installed
    if studio_is_installed() {
        send_log(
            &tx,
            "info",
            "loka-update",
            "Loka Studio is installed — checking for Studio update...",
        );
...
async fn check_latest_version() -> Option<String> {
    ...
    let resp = client
        .get(&url)
        .header("User-Agent", format!("loka/{}", env!("CARGO_PKG_VERSION")))
        .send()
        .await
        .ok()?;
...
async fn perform_update(latest: &str) -> Result<(), String> {
    ...
    let resp = client
        .get(&url)
        .header("User-Agent", format!("loka/{}", current))
        .send()
        .await
    ...
    let asset_url = get_asset_url(&assets).ok_or_else(|| {
        format!(
            "No pre-built binary for this platform. Update manually: \
             cargo install --git https://github.com/{} loka-cli",
            GITHUB_REPO
        )
    })?;
...
fn get_asset_url(assets: &[Value]) -> Option<String> {
    ...
    for asset in assets {
        if let Some(name) = asset["name"].as_str() {
            // Match the CLI asset (e.g. "loka-windows-x64.zip") but not
            // the Studio asset (e.g. "loka-studio-windows-x64.zip")
...
/// Get the directory where Loka Studio is (or should be) installed.
/// Located alongside the loka binary: `<binary-dir>/loka-studio/`.
...
fn studio_executable() -> Result<std::path::PathBuf, String> {
    ...
    if cfg!(target_os = "windows") {
        Ok(dir.join("loka_studio.exe"))
    } else if cfg!(target_os = "macos") {
        Ok(dir
            .join("loka_studio.app")
            .join("Contents")
            .join("MacOS")
            .join("loka_studio"))
    } else {
        Ok(dir.join("loka_studio"))
    }
}

/// Check if Loka Studio is installed.
fn studio_is_installed() -> bool {
    studio_executable().map(|p| p.exists()).unwrap_or(false)
}

/// Get the asset name pattern for Loka Studio on this platform.
fn studio_asset_target() -> &'static str {
    if cfg!(target_os = "windows") {
        "loka-studio-windows"
    } else if cfg!(target_os = "macos") {
        if cfg!(target_arch = "aarch64") {
            "loka-studio-macos-arm64"
        } else {
            "loka-studio-macos-x64"
        }
    } else {
        "loka-studio-linux"
    }
...
/// Download and install Loka Studio from GitHub releases.
async fn download_studio_from_github(
    ...
    let asset_url = assets
        ...
        .ok_or_else(|| {
            format!(
                "No Loka Studio build for this platform ({}) in the latest release. \
                 Build from source: cd loka-studio && flutter build",
                target
            )
        })?;
    ...
    send_log(
        ...
        "info",
        "loka-studio",
        &format!("Downloading Loka Studio {} for {}...", version, target),
    );
    ...
    Ok(format!(
        "Loka Studio {} installed successfully at {}",
        version,
        install_dir.display()
    ))
}
...
async fn tool_download_studio(notify_tx: &mpsc::UnboundedSender<Value>) -> Result<String, String> {
    if studio_is_installed() {
        send_log(
            ...
            "info",
            "loka-studio",
            "Loka Studio already installed. Re-downloading to get latest version...",
        );
    }
...
async fn tool_launch_studio(
    ...
    if !studio_is_installed() {
        send_log(
            ...
            "info",
            "loka-studio",
            "Loka Studio not found. Downloading...",
        );
        download_studio_from_github(notify_tx).await?;
    }
    ...
    if !exe.exists() {
        return Err(format!(
            "Studio executable not found at {}. The release archive may have a different structure. \
             Check the loka-studio directory and launch manually.",
            exe.display()
        ));
    }
    ...
    let endpoint = if let Some(ep) = args.get("endpoint").and_then(|v| v.as_str()) {
        ...
        // Start a background `loka serve` pointing at the same .sdb file.
        let data_dir = ctx.data_dir.as_deref().unwrap_or("./loka-data");
        ...
        let loka_exe =
            std::env::current_exe().map_err(|e| format!("Cannot find loka binary: {}", e))?;
        ...
        match std::process::Command::new(&loka_exe)
            .args(&serve_args)
            .spawn()
        {
            Ok(_) => {
            ...
            Err(e) => {
                return Err(format!(
                    "Failed to start background server for Studio: {}. \
                     Start a server manually with `loka serve --data-dir {}` \
                     and try again.",
                    e, data_dir
                ));
            }
    ...
    send_log(
        ...
        "info",
        "loka-studio",
        &format!("Launching Loka Studio (connecting to {})...", endpoint),
    );
        ...
        Some(p) if !p.applied => {
            ...
            Ok(format!(
                "Auto-update to v{} declined for this session. \
                 Run `loka update` manually to install later.",
                p.latest_version
            ))
        }
...
fn open_serverless(
    ...
    (
        loka_core::PersistentStore,
        loka_core::TripleStore,
        loka_core::TermDictionary,
    ),
    String,
> {
    let ps = loka_core::PersistentStore::open(data_dir)
...
async fn tool_health_report(ctx: &McpContext) -> Result<String, String> {
    if ctx.mode == "serverless" {
        ...
        let report = loka_sparql::generate_health_report(&store, &dict, &vectors, None);
...
async fn tool_rebuild_hnsw(
    ...
    send_log(notify_tx, "info", "loka-hnsw", "HNSW rebuild complete.");
    Ok(result.to_string())
}
        ...
        if result.columns.len() == 1 && result.columns[0] == "result" {
            ...
            if let Some(row) = result.rows.first() {
                if let Some(&id) = row.get("result") {
                    if loka_core::decode_inline_boolean(id) == Some(true) {
...
async fn tool_backup(ctx: &McpContext) -> Result<String, String> {
    ...
    let backup_file = format!("loka_backup_{}.ttl", timestamp);
    ...
    Ok(format!(
        "Backup exported to {} ({} bytes). For full disk-level backup, \
         use --backup-interval on loka serve or copy the data directory.",
        backup_file,
        content.len()
    ))
}
...
async fn tool_vector_search(ctx: &McpContext, args: &Value) -> Result<String, String> {
    ...
    let query = format!(
        "SELECT ?s WHERE {{ ?s <{}> ?vec . VECTOR_SIMILAR(?s <{}> \"{}\"^^<http://loka.dev/f32vec>, {}) }} LIMIT {}",
