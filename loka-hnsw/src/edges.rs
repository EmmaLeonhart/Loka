...
//! ## Edge types
//!
//! **Horizontal edges** (`loka:hnswHorizontalNeighbor`):
...
//!
//! **Vertical edges** (`loka:hnswLayerDescend`):
...
//! ?nodeA loka:hnswHorizontalNeighbor ?nodeB .
//! << ?nodeA loka:hnswHorizontalNeighbor ?nodeB >> loka:hnswLayer 2 .
//! << ?nodeA loka:hnswHorizontalNeighbor ?nodeB >> loka:similarity 0.95 .
//!
//! # Vertical descent edge (layer L → layer L-1):
//! ?node loka:hnswLayerDescend ?node .
//! << ?node loka:hnswLayerDescend ?node >> loka:hnswFromLayer 3 .
//! << ?node loka:hnswLayerDescend ?node >> loka:hnswToLayer 2 .
//! ```
//!
//! The generic `loka:hnswNeighbor` predicate is preserved for backward
...
pub const HNSW_NEIGHBOR_IRI: &str = "http://loka.dev/hnswNeighbor";
...
pub const HNSW_HORIZONTAL_NEIGHBOR_IRI: &str = "http://loka.dev/hnswHorizontalNeighbor";
...
pub const HNSW_LAYER_DESCEND_IRI: &str = "http://loka.dev/hnswLayerDescend";

/// Layer metadata predicate (used in RDF-star annotations).
pub const HNSW_LAYER_IRI: &str = "http://loka.dev/hnswLayer";

/// Similarity score metadata predicate (used in RDF-star annotations).
pub const HNSW_SIMILARITY_IRI: &str = "http://loka.dev/hnswSimilarity";

/// Source predicate metadata (which vector predicate this edge belongs to).
pub const HNSW_PREDICATE_IRI: &str = "http://loka.dev/hnswPredicate";
...
///
/// Represents either:
/// - Horizontal: `source loka:hnswHorizontalNeighbor target` (different nodes, same layer)
/// - Vertical: `node loka:hnswLayerDescend node` (same node, adjacent layers)
///
