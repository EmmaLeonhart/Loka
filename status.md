...
> Loka engine + RDF-star world-model loop. Project is being rebranded **Loka** on Hugging Face; the GitHub repo will be renamed later.
...
|---|---|---|
| `bwwrjw7on` | `./target/release/loka.exe serve --port 3030` | Live, serving 5,055,385 triples |
| `bgju84u0v` | v5 training (d_model 512, 6 layers, 44M params) | Mid epoch 1, ETA ~2 h to 5/5 |

### Stored corpus
`loka-data/db` (~770 MB) holds 5.05M RDF-star triples + 1.69M annotations on quoted-triple subjects. Source: `philippesaade/wikidata` HF dataset, 27,780 entities, every language label, every Wikidata qualifier and reference preserved.

A cold backup lives at `loka-data-backup-2026-05-09/` (frozen, no live writers — use this for upload sources).
...
### Hugging Face mirror
`https://huggingface.co/datasets/EmmaLeonhart/loka` — corpus + checkpoints already uploaded. **`loka-data/` folder and `v4` tag missing** (first upload was blocked by the file-lock from loka serve; retry blocked by credential-leak guard after token was pasted in chat). To finish:
...
3. `python tools/hf_snapshot.py --user EmmaLeonhart --snapshot-name v4 --loka-data-path loka-data-backup-2026-05-09`
...
#    Add --post to also POST the generated triples into Loka.

# 6. Snapshot to Hugging Face
python tools/hf_snapshot.py --user EmmaLeonhart --snapshot-name v5 \
  --loka-data-path loka-data-backup-2026-05-09
```
...
- **`http://loka.dev/provenance/` is a reserved namespace.** The world model never sees, proposes, or emits predicates under it. Enforced at three layers in `preprocess.py` and `infer_with_citations.py`. See `feedback_provenance_namespace_reserved` memory.
...
Known issues:
- **Loka SPARQL occasionally returns literal values in the predicate position.** Filtered out at preprocess; a real engine bug to fix later. ~1% of rows.
- **Loka `/triples` endpoint wedges after ~1M write-flush cycles.** Recoverable by restart (data persists). Hit it twice during the 5M ingest. Real engine bug.
...
5. **`--post` inference back into Loka.** Loop-closing experiment: model generates triples → land in store → retrain on the bigger corpus that includes its own (filtered) outputs.
