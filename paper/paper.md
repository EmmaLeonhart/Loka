...
We also surface two engine-level bugs found at scale: a SPARQL quirk producing literal values in the predicate slot, and a write-flush wedge in the persistent layer at roughly every 5–6× growth in stored triples. Both are now resolved: the wedge was diagnosed and fixed during v9 (§5.7) by batching the persistent-store writes into a single sled multi-tree transaction per HTTP request (verified at 2 M-triple sustained ingest with no recurrence); the literal-predicate quirk was masked at preprocess and then fixed at the query layer (§6.1) by enforcing in the `loka-sparql` executor the RDF invariant that the predicate position never binds a literal, with regression tests and the full engine suite green. The ingest-side root cause — a content-addressed quoted-triple id with no persisted reverse map — is specified as the next engine work item (`planning/cascade-retraction.md`, Phase 0).
...
- **SPARQL `?s ?p ?o` occasionally returns literal values in the predicate slot.** *(Query layer resolved 2026-05-16; ingest-side root cause scoped.)* On ~1% of rows of a 5 M-triple corpus a plain `?s ?p ?o` scan surfaced a literal in the `?p` slot, and the RDF-star annotation pattern `<< ?s ?p ?o >> ?qp ?qv` surfaced a literal in `?qp` — invalid RDF, the fingerprint of a mis-keyed RDF-star annotation row. It was masked downstream at preprocess (dropping the ~1%). RDF and RDF-star both forbid literals in the predicate position, so the fix moves the invariant into the engine: the `loka-sparql` executor now refuses to bind a literal (inline literal *or* quoted-string term) into the predicate position, applied at the basic-scan candidate loop and at both RDF-star wildcard outer-predicate binding loops. Commit `b63af81`; two regression tests (`engine_bug2_literal_predicate_rows_are_never_surfaced`, `engine_bug2_rdf_star_annotation_round_trips_without_literal_predicate`); full 119-test `loka-sparql` suite green. The investigation also identified the *ingest-side* root cause: `quoted_triple_id` is a content hash with no persisted reverse map, so a quoted-triple subject cannot be rendered faithfully and the proto bulk path persists a `<<QUOTED_TRIPLE>>` sentinel string instead. That reverse index is specified in `planning/cascade-retraction.md` as Phase 0 (it is also the gating prerequisite for provenance cascade-retraction); until it lands the query layer is honest but RDF-star round-trips through a re-ingested store are lossy. The query-layer guard makes the database honest regardless of how a malformed row entered the store, so the preprocess mask is now redundant against a current-engine instance (the v11–v14 corpora were built with it and are unaffected).
