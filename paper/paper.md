...
We also surface two engine-level bugs found at scale: a SPARQL quirk producing literal values in the predicate slot, and a write-flush wedge in the persistent layer at roughly every 5–6× growth in stored triples. Both are now resolved: the wedge was diagnosed and fixed during v9 (§5.7) by batching the persistent-store writes into a single sled multi-tree transaction per HTTP request (verified at 2 M-triple sustained ingest with no recurrence); the literal-predicate quirk was fully resolved (§6.1) on both the query layer (a `loka-sparql` executor invariant: the predicate position never binds a literal) and the ingest layer (a persisted reverse index for content-addressed quoted-triple ids, plus two ingest paths corrected), so RDF-star now round-trips losslessly across ingest, persistence, query, and export. Closing the ingest side also unblocked **cascade-retraction** (§6.3): the provenance graph is now not only auditable but *actionable*.
...
2. **Drop rows with non-URI predicates.** ~1% of rows on a 5M corpus exhibited a Loka SPARQL bug (§6.1) where literal values surfaced in the `?p` slot. RDF disallows literal predicates, so dropping is safe. The v11–v14 corpora were built with this drop in place; the underlying bug was subsequently fixed at the query layer (§6.1), so on a current-engine instance the executor never emits such a row and this step is now a redundant safety net rather than a load-bearing filter.
...
**Cross-version comparison.** v13 at 242.75 lands between v12's lost best (epoch-4 ppl 226.86, never shipped) and v12's actual shipped value (epoch-6 ppl 250.82, training-disruption-degraded). The corpus-quality lever (more data, same architecture) is therefore still working: v11 (350 k) → v12 (672 k) → v13 (2 511 k) trained best results 279.12 → 226.86 → 242.75 — but the trained-headroom-on-this-laptop floor has clearly bottomed out around 240. Contributors with desktop GPUs and bigger batch sizes are the natural next axis; see the GPU-donation contributor path documented in the repository README's "Contributing GPU time" section and `tools/contribute_v14_training.py`.
...
- **SPARQL `?s ?p ?o` occasionally returns literal values in the predicate slot.** *(Fully resolved 2026-05-16, ingest + query layers.)* On ~1% of rows of a 5 M-triple corpus a plain `?s ?p ?o` scan surfaced a literal in the `?p` slot, and the RDF-star annotation pattern `<< ?s ?p ?o >> ?qp ?qv` surfaced a literal in `?qp` — invalid RDF, the fingerprint of a mis-keyed RDF-star annotation row. The fix has three parts. (i) **Query-layer invariant:** RDF and RDF-star forbid literals in the predicate position, so the `loka-sparql` executor now refuses to bind a literal (inline literal *or* quoted-string term) into the predicate position, applied at the basic-scan candidate loop and at both RDF-star wildcard outer-predicate binding loops (commit `b63af81`; two `engine_bug2_*` regression tests). (ii) **Ingest-side root cause:** `quoted_triple_id` is a content hash and the store kept no persisted reverse map, so a quoted-triple subject could not be rendered back to `<< s p o >>` (the proto bulk path persisted a `<<QUOTED_TRIPLE>>` sentinel) and a provenance cascade could not dereference a `propositionInferredFrom` source — a one-way hash cannot be reversed. A persisted `quoted_triple_id → (s,p,o)` reverse index (`loka-core`, written inside the existing batch transaction, rehydrated on reopen) closes this; a quoted position now renders faithfully across query, CSV, and Turtle/N-Triples export, and survives a store reopen. (iii) **Two ingest paths** (`loka-ffi`, the serverless MCP tool) that used the non-star line parser and silently dropped inner triples were switched to the RDF-star parser. RDF-star now round-trips losslessly across ingest, persistence, query, and export; the preprocess mask is redundant against a current-engine instance (the v11–v14 corpora were built with it and are unaffected). Closing the ingest side also unblocked cascade-retraction (§6.3).
