...
/// This is analogous to scanning a SQL table — Loka auto-discovers
    ...
    for &n in &[500, 2_000] {
        ...
        group.bench_with_input(criterion::BenchmarkId::new("persons", n), &(), |b, _| {
            b.iter(|| {
                ...
                let registry = loka_core::discover_pseudo_tables(black_box(&node_props), &store);
                black_box(registry);
            });
        });
    }
                ...
                for seg in &table.segments {
                    for (col_idx, col_prop) in table.columns.iter().enumerate() {
                        if col_prop.predicate == category {
                            let results = loka_core::pseudotable::scan_column_eq(
                                seg,
                                col_idx,
                                black_box(cats[3]),
                            );
