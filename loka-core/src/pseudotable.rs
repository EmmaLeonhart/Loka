...
// ---------------------------------------------------------------------------
// Public SIMD dispatch — runtime feature detection (same pattern as loka-hnsw)
// ---------------------------------------------------------------------------
...
pub fn discover_pseudo_tables(
    ...
    exact_vec.sort_by_key(|x| std::cmp::Reverse(x.1.len()));
...
fn mine_depth2_paths(store: &TripleStore) -> Vec<SubgraphPattern> {
    ...
    groups.sort_by_key(|x| std::cmp::Reverse(x.1.len()));
