@echo off
REM Build the Loka Protege plugin and install it.
...
echo Building Loka Protege plugin...
cd /d "%~dp0"
call mvn package -q
if errorlevel 1 (
    echo Build failed!
...
copy /y target\loka-protege-plugin-0.1.0.jar "%PROTEGE_DIR%\plugins\"
if errorlevel 1 (
...
echo.
echo Done! Restart Protege, then go to Window ^> Tabs ^> Loka
echo.
