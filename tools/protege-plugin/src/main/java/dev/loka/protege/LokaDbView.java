...
/**
 * Protégé view component that connects to a running Loka instance.
 *
 * Features:
 * - Check/start Loka server
 * - Load ontology from Loka into Protégé
 * - Save current ontology back to Loka
 * - Validate data against OWL constraints
 */
public class LokaDbView extends AbstractOWLViewComponent {
    ...
    protected void initialiseOWLView() throws Exception {
        ...
        connectionPanel.setBorder(new TitledBorder("Loka Connection"));
        ...
        connectButton.setToolTipText("Check if Loka is running; start it if not");
        connectButton.addActionListener(this::onConnect);
        buttonPanel.add(connectButton);

        loadButton = new JButton("Load from Loka");
        loadButton.setToolTipText("Load the ontology from Loka into Protégé");
        loadButton.addActionListener(this::onLoad);
        loadButton.setEnabled(false);
        buttonPanel.add(loadButton);

        saveButton = new JButton("Save to Loka");
        saveButton.setToolTipText("Export the current ontology as Turtle and upload to Loka");
        saveButton.addActionListener(this::onSave);
        saveButton.setEnabled(false);
        buttonPanel.add(saveButton);

        validateButton = new JButton("Validate");
        validateButton.setToolTipText("Run OWL verification queries against Loka");
        ...
        log("Loka plugin ready. Click 'Connect / Start' to begin.");
    }

    @Override
    protected void disposeOWLView() {
        // Clean up the Loka process if we started it
        if (lokaProcess != null && lokaProcess.isAlive()) {
            log("Shutting down Loka...");
            lokaProcess.destroy();
        }
    }
    ...
    private void onConnect(ActionEvent e) {
        new SwingWorker<Boolean, Void>() {
            ...
            protected Boolean doInBackground() {
                ...
                if (checkHealth()) {
                    log("Loka is already running at " + getBaseUrl());
                    return true;
                }

                // Try to start it
                log("Loka not running. Attempting to start...");
                return tryStartLoka();
            }
    ...
    private boolean tryStartLoka() {
        try {
            // Try to find loka executable
            String port = portField.getText().trim();
            ProcessBuilder pb = new ProcessBuilder("loka", "serve", "--port", port);
            pb.redirectErrorStream(true);
            lokaProcess = pb.start();

            // Read output in background
            Thread outputReader = new Thread(() -> {
                try (BufferedReader br = new BufferedReader(
                        new InputStreamReader(lokaProcess.getInputStream(), StandardCharsets.UTF_8))) {
                    String line;
            ...
            for (int i = 0; i < 20; i++) {
                Thread.sleep(500);
                if (checkHealth()) {
                    log("Loka started successfully on port " + port);
                    return true;
                }
            }

            log("Loka failed to start within 10 seconds.");
            log("Make sure 'loka' is on your PATH, or start it manually.");
            return false;
        } catch (Exception e) {
            log("Could not start Loka: " + e.getMessage());
            log("Start it manually: loka serve --port " + portField.getText().trim());
            return false;
        }
    }
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() {
                try {
                    ...
                    File tempFile = File.createTempFile("loka-", ".ttl");
                    ...
                    log("Loaded ontology: " + axiomCount + " axioms from Loka.");
    ...
    private void onValidate(ActionEvent e) {
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() {
                try {
                    log("Running OWL validation against Loka...");
