 ...
 * application-level errors returned by the Loka server.</p>
 */
public class LokaError extends RuntimeException {

    private final int statusCode;

    /**
     * Create a new LokaError with a message and HTTP status code.
     *
     * @param message    human-readable error description
     * @param statusCode the HTTP status code, or -1 if the error is not HTTP-related
     */
    public LokaError(String message, int statusCode) {
        super(message);
        this.statusCode = statusCode;
    }

    /**
     * Create a new LokaError wrapping another throwable.
     *
     * @param message human-readable error description
     * @param cause   the underlying cause
     */
    public LokaError(String message, Throwable cause) {
        super(message, cause);
        this.statusCode = -1;
    }
