...
/// Errors that can occur when communicating with a Loka instance.
#[derive(Debug, Error)]
pub enum LokaError {
    ...
    #[error("Loka server error ({status}): {message}")]
    Server {
        status: u16,
        message: String,
    },
...
/// A convenience alias for `std::result::Result<T, LokaError>`.
