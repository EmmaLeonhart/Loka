...
class LokaError(Exception):
    """Exception raised by the Loka client on request failures."""
...
class LokaClient:
    ...
    Args:
        endpoint: Base URL of the Loka HTTP server.
    ...
    def __init__(
        ...
        self._session.headers.update({"User-Agent": "loka-python/0.1.0"})
    ...
    def _request(
        ...
        *,
        ...
        """Send an HTTP request and raise :class:`LokaError` on failure."""
        try:
            ...
            raise LokaError(f"Connection error: {exc}") from exc
    ...
    def sparql(self, query: str) -> dict:
        ...
        Raises:
            LokaError: If the server returns a non-2xx status code.
        """
        resp = self._request(
            "GET",
            "/sparql",
    ...
    def declare_vector(
        ...
        Raises:
            LokaError: If the server rejects the declaration.
        """
        resp = self._request(
            "POST",
            "/vectors/declare",
    ...
    def insert_vector(
        ...
        Raises:
            LokaError: If the server rejects the insert.
        """
        resp = self._request(
            "POST",
            "/vectors",
