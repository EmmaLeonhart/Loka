"""LangChain integration for Loka.

Provides Loka as both a VectorStore and a knowledge graph for
Retrieval-Augmented Generation (RAG) pipelines.

Usage:
    from loka.langchain import LokaVectorStore

    vectorstore = LokaVectorStore(
        endpoint="http://localhost:3030",
        predicate="http://loka.dev/hasEmbedding",
    )
...
from .client import LokaClient


class LokaVectorStore(VectorStore):
    """LangChain VectorStore backed by Loka.

    Uses Loka's HNSW vector index for similarity search and
    ...
    def __init__(
        ...
        predicate: str = "http://loka.dev/hasEmbedding",
        embedding: Optional[Embeddings] = None,
        dimensions: int = 1024,
        **kwargs: Any,
    ):
        self._client = LokaClient(endpoint, owl_validation=False)
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        """Add texts with embeddings to Loka."""
        ...
        for i, (text, vector) in enumerate(zip(texts_list, vectors)):
            ...
            subject = f"http://loka.dev/doc/{text_hash}"
            ...
            ntriples = f'<{subject}> <http://loka.dev/text> "{escaped}" .'

            # Insert metadata
            if metadatas and i < len(metadatas):
                for key, value in metadatas[i].items():
                    escaped_val = str(value).replace('"', '\\"')
                    ntriples += f'\n<{subject}> <http://loka.dev/meta/{key}> "{escaped_val}" .'
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        sparql = (
            ...
            f'"{vec_str}"^^<http://loka.dev/f32vec>, 0.5, k:={k})\n'
            f'  OPTIONAL {{ ?doc <http://loka.dev/text> ?text }}\n'
            f'}}'
        )
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        """Create a LokaVectorStore from texts."""
