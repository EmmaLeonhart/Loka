...
| File | Purpose |
|---|---|
| `preprocess.py` | Pull triples from SutraDB via SPARQL, resolve IRIs to English labels, emit a flat training file. |
...
- **No write-back to SutraDB yet.** The trained model produces predictions but they are not written back as inferred triples with provenance. That is the next milestone.
