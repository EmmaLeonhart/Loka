...
# Wikidata external-identifier predicates. These are properties whose datatype
# on Wikidata is `external-id` — Freebase IDs, ISNI, GND, LCCN, Dewey codes,
# the works. There are ~10,200 of them, most of which appear rarely. Two
# reasons to exclude them from the training corpus:
#
#   1. They are catalog cross-references, not semantic facts. The model
#      learning that "Q42 has Library of Congress authority ID n79115075"
#      teaches it format pattern (LCCN looks like /n\d{8}/) without anything
#      semantically transferable. v6 inference confirms this: the model
#      memorised the *shape* of these IDs and now hallucinates them on prompt
#      ("ISNI -> 00000000", "Freebase ID -> /m/0c__9"), and worse, leaks the
#      shape onto unrelated predicates ("instance of -> + Ġof - 00 - 03 T 00").
#   2. They are ~half of all Wikidata triples by volume. Training on them
#      means half of model capacity goes to learning catalog formats.
#
# The list is generated by `tools/refresh_wikidata_external_id_list.py` (or
# inline in commit history). Loaded lazily so importing this module doesn't
# touch disk unless the filter is actually used.
EXTERNAL_ID_LIST_PATH = Path(__file__).parent / "wikidata_external_id_predicates.json"

_EXTERNAL_ID_IRIS_CACHE: set[str] | None = None


def load_external_id_iris() -> set[str]:
    """Return the set of Wikidata-property IRIs whose datatype is external-id.

    Cached on first call. Returns an empty set if the list file is missing
    so callers in environments without it just don't filter.
    """
    global _EXTERNAL_ID_IRIS_CACHE
    if _EXTERNAL_ID_IRIS_CACHE is not None:
        return _EXTERNAL_ID_IRIS_CACHE
    if not EXTERNAL_ID_LIST_PATH.exists():
        _EXTERNAL_ID_IRIS_CACHE = set()
        return _EXTERNAL_ID_IRIS_CACHE
    data = json.loads(EXTERNAL_ID_LIST_PATH.read_text(encoding="utf-8"))
    _EXTERNAL_ID_IRIS_CACHE = {
        f"http://www.wikidata.org/prop/direct/{pid}" for pid in data.get("pids", [])
    }
    return _EXTERNAL_ID_IRIS_CACHE


def is_external_id_predicate(iri: str) -> bool:
    """True if `iri` is a Wikidata external-identifier predicate."""
    return iri in load_external_id_iris()
...
def main() -> None:
    ...
    parser.add_argument(
        "--keep-external-ids",
        action="store_true",
        help="Include Wikidata external-identifier predicates in the corpus. "
             "Default is to exclude them — the v6 model demonstrated that "
             "training on Freebase IDs / ISNI / LCCN / Dewey codes etc. teaches "
             "the model catalog format patterns that then leak onto unrelated "
             "predicates ('instance of -> + Ġof - 00 - 03 T 00').",
    )
    ...
    ext_id_iris = set() if args.keep_external_ids else load_external_id_iris()
    if ext_id_iris:
        print(
            f"Excluding Wikidata external-identifier predicates "
            f"({len(ext_id_iris):,} property IRIs)",
            file=sys.stderr,
        )
    ...
    print(
        ...
        f"{skipped_external_id:,} (Wikidata external-identifier predicates), "
