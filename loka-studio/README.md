...
Visual database management client for Loka — like MongoDB Compass, but for RDF-star triplestores with HNSW vector indexes.

Built with Flutter for desktop (macOS, Windows, Linux), web, and mobile.

## Why a visual client?

Most Loka usage happens through AI agents and application SDKs. The visual client exists for things humans are better at:
...
- Flutter SDK >= 3.2.0
- A running Loka instance (default: `http://localhost:3030`)
...
├── services/
│   ├── loka_client.dart      # Dart HTTP client (mirrors TS SDK)
...
└── theme/
    └── loka_theme.dart       # Dark theme (matches tools/browse.html)
```
