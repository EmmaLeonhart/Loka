...
/// Dart client for the Loka HTTP API.
///
...
class LokaClient {
  ...
  LokaClient({ConnectionConfig? config, http.Client? httpClient})
  ...
  Future<SparqlResult> query(String sparql) async {
    ...
    if (res.statusCode != 200) {
      throw LokaClientException('Query failed: ${res.statusCode} ${res.body}');
    }
...
class LokaClientException implements Exception {
  final String message;
  const LokaClientException(this.message);
  @override
  String toString() => 'LokaClientException: $message';
}
