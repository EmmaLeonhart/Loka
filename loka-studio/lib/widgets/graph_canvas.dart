    ...
    for (final edge in edges) {
      ...
      switch (edge.type) {
        case EdgeType.semantic:
          paint.color = LokaTheme.edgeSemantic;
        case EdgeType.vector:
          paint.color = LokaTheme.edgeVector;
        case EdgeType.hnswNeighbor:
          paint.color = LokaTheme.edgeHnsw;
      ...
      if (zoom > 0.6 && edge.label.isNotEmpty) {
        ...
        final tp = TextPainter(
          text: TextSpan(
            text: edge.label,
            style: TextStyle(
              color: LokaTheme.muted.withOpacity(0.7),
              fontSize: 9 / zoom,
            ),
          ),
    ...
    for (final node in nodes) {
      ...
      switch (node.type) {
        case NodeType.entity:
          color = LokaTheme.nodeEntity;
        case NodeType.literal:
          color = LokaTheme.nodeLiteral;
        case NodeType.blankNode:
          color = LokaTheme.nodeBlank;
        case NodeType.vectorLiteral:
          color = LokaTheme.nodeVector;
      }
