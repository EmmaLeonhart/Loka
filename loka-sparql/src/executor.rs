...
fn execute_query_with_ctx(query: &Query, ctx: &mut ExecutionContext<'_>) -> Result<QueryResult> {
    ...
    if query.query_type == QueryType::Ask {
        ...
        row.insert(
            "result".to_string(),
            if has_results {
                loka_core::inline_boolean(true)
            } else {
                loka_core::inline_boolean(false)
            },
        );
        return Ok(QueryResult {
    ...
    match pattern {
        ...
        Pattern::MetricSearch {
            ...
            let hnsw_metric = match metric {
                SearchMetric::Cosine => loka_hnsw::DistanceMetric::Cosine,
                SearchMetric::Euclidean => loka_hnsw::DistanceMetric::Euclidean,
                SearchMetric::DotProduct => loka_hnsw::DistanceMetric::DotProduct,
            };
            evaluate_vector_similar(
                subject,
                predicate,
...
///
/// Supports:
/// - TypedLiteral with xsd:dateTime or loka:temporal datatype
    ...
    match term {
        ...
        Term::Variable(name) => {
            if let Some(&id) = row.get(name) {
                ...
                if let Some(tv) = loka_core::decode_inline_temporal(id) {
...
fn evaluate_vector_similar(
    ...
    top_k: Option<usize>,
    metric_override: Option<loka_hnsw::DistanceMetric>,
    current: &[Bindings],
    current_scores: &[HashMap<String, f32>],
    ctx: &mut ExecutionContext<'_>,
) -> Result<(Vec<Bindings>, Vec<HashMap<String, f32>>)> {
...
fn evaluate_triple_pattern(
    ...
    // When config says Virtual mode, intercept loka:hnswNeighbor and typed
        ...
        } = subject
        {
            ...
            if is_id.is_some() && ip_id.is_some() && io_id.is_some() {
                ...
                for inner_triple in &inner_candidates {
                    ...
                    let qt_id = loka_core::quoted_triple_id(
...
fn resolve_term(
    ...
    match term {
        ...
        Term::IntegerLiteral(n) => Ok(loka_core::inline_integer(*n)),
        Term::TypedLiteral { value, datatype } => {
        ...
        Term::QuotedTriple {
            ...
            match (s, p, o) {
                (Some(s_id), Some(p_id), Some(o_id)) => {
                    Ok(Some(loka_core::quoted_triple_id(s_id, p_id, o_id)))
                }
                _ => Ok(None),
            }
        }
...
///
/// - `loka:hnswHorizontalNeighbor` — only horizontal edges (same-layer neighbors)
/// - `loka:hnswLayerDescend` — only vertical descent edges (layer L → L-1)
/// - `loka:hnswNeighbor` — ALL edges (backward compatible, matches both types)
///
/// The typed predicates enable SPARQL property paths to express HNSW search:
/// ```sparql
/// ?entry loka:hnswLayerDescend* / loka:hnswHorizontalNeighbor+ ?result
/// ```
fn try_evaluate_hnsw_edge_pattern(
    ...
    let edge_type_filter: Option<Option<loka_hnsw::HnswEdgeType>> = match resolved_iri.as_deref() {
        // Generic predicate: match ALL edge types (no filter)
        Some(iri) if iri == loka_hnsw::HNSW_NEIGHBOR_IRI => Some(None),
        // Typed predicates: match only the specific edge type
        Some(iri) if iri == loka_hnsw::HNSW_HORIZONTAL_NEIGHBOR_IRI => {
            Some(Some(loka_hnsw::HnswEdgeType::Horizontal))
        }
        Some(iri) if iri == loka_hnsw::HNSW_LAYER_DESCEND_IRI => {
            Some(Some(loka_hnsw::HnswEdgeType::VerticalDescend))
        }
    ...
    let neighbor_pred_id = ctx.dict.lookup(loka_hnsw::HNSW_NEIGHBOR_IRI).unwrap_or(0);
...
fn evaluate_filter(expr: &FilterExpr, row: &Bindings, ctx: &mut ExecutionContext<'_>) -> bool {
    match expr {
        ...
        FilterExpr::DatatypeEquals(var, expected_dt) => {
            if let Some(&id) = row.get(var) {
                if loka_core::decode_inline_integer(id).is_some() {
                    return expected_dt.contains("integer");
                }
                if loka_core::decode_inline_boolean(id).is_some() {
...
fn apply_group_by_and_aggregates(
    ...
    if group_by.is_empty() && !aggregates.is_empty() {
        ...
        for agg in aggregates {
            ...
            if let Some(id) = loka_core::inline_integer(value) {
        ...
        AggregateFunction::Sum | AggregateFunction::Avg => {
            let values: Vec<i64> = match &agg.argument {
                AggregateArg::Variable(v) => rows
                    ...
                    .filter_map(|&id| loka_core::decode_inline_integer(id))
                    .collect(),
...
mod tests {
    ...
    fn setup() -> (TripleStore, TermDictionary) {
        ...
        let age_30 = loka_core::inline_integer(30).unwrap();
    ...
    fn vector_similar_unbound_subject() {
        ...
        let q = parser::parse(
            "SELECT ?doc WHERE { \
             VECTOR_SIMILAR(?doc <http://example.org/hasEmbedding> \"1.0 0.0 0.0\"^^<http://loka.dev/f32vec>, 0.8) \
             }",
        )
        .unwrap();
    ...
    fn vector_similar_with_graph_join() {
        ...
        let vec1_id = dict.intern("\"vec_d1\"^^<http://loka.dev/f32vec>");
        let vec2_id = dict.intern("\"vec_d2\"^^<http://loka.dev/f32vec>");
        let vec3_id = dict.intern("\"vec_d3\"^^<http://loka.dev/f32vec>");
        store
            .insert(Triple::new(doc1, has_embedding, vec1_id))
            .unwrap();
        store
        ...
        let q = parser::parse(
            "PREFIX ex: <http://example.org/> \
             ...
             ?doc a ex:Paper . \
             ?doc ex:title ?title . \
             VECTOR_SIMILAR(?doc ex:hasEmbedding \"1.0 0.0 0.0\"^^<http://loka.dev/f32vec>, 0.5) \
             }",
        )
        .unwrap();
    ...
    fn setup_temporal() -> (TripleStore, TermDictionary) {
        use loka_core::TemporalSignifier;
