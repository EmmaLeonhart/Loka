...
//! without prior knowledge of Loka internals. An agent should be able to
//! read the health report and immediately know which subsystems need attention.
//!
//! # Example output (CLI, `loka health`)
//!
//! ```text
//! # Loka Health Report
//!
...
impl HealthReport {
    ...
    pub fn to_ai_text(&self) -> String {
        ...
        sections.push("# Loka Health Report".to_string());
...
pub fn generate_health_report(
    ...
    let pseudo_table_metrics = match pseudo_tables {
        ...
        None => PseudoTableHealthMetrics {
            ...
            status_detail:
                "Pseudo-tables not yet discovered. Run `loka health --refresh` to populate."
                    .to_string(),
        },
    };
...
fn compute_layer_distribution(index: &loka_hnsw::HnswIndex, max_layer: u8) -> Vec<usize> {
...
fn compute_connectivity_metrics(index: &loka_hnsw::HnswIndex) -> (f64, usize, usize) {
...
mod tests {
    ...
    use loka_hnsw::{DistanceMetric, VectorPredicateConfig};
    ...
    fn report_to_ai_text_is_readable() {
        ...
        assert!(text.contains("# Loka Health Report"));
