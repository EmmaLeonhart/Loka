...
pub enum Pattern {
    ...
    /// FILTER(expression)
    Filter(FilterExpr),
    /// VECTOR_SIMILAR(?var predicate "vector"^^loka:f32vec, threshold)
    ...
    ///
    /// COSINE_SEARCH(?var predicate "vector"^^loka:f32vec, threshold)
    /// EUCLID_SEARCH(?var predicate "vector"^^loka:f32vec, threshold)
    /// DOTPRODUCT_SEARCH(?var predicate "vector"^^loka:f32vec, threshold)
    ///
...
pub enum Term {
    ...
    IntegerLiteral(i64),
    /// A vector literal: "0.1 0.2 0.3"^^loka:f32vec
...
impl<'a> Parser<'a> {
    ...
    fn parse_vector_similar(&mut self) -> Result<Pattern> {
        ...
        // Parse the vector literal: "0.1 0.2 0.3"^^<loka:f32vec>
    ...
    ///
    /// Grammar: KEYWORD(?subject predicate "vector"^^loka:f32vec, threshold [, ef:=N] [, k:=N])
    ///
    ...
    /// Expects: "0.1 0.2 0.3"^^<http://loka.dev/f32vec> or "0.1 0.2 0.3"^^<loka:f32vec>
    fn parse_vector_literal_value(&mut self) -> Result<Vec<f32>> {
        ...
        let datatype = if self.peek_char() == Some('<') {
            ...
            // Try prefixed name like loka:f32vec
        ...
        if datatype != "http://loka.dev/f32vec" && datatype != "loka:f32vec" {
            return Err(self.error(&format!("expected loka:f32vec datatype, got {}", datatype)));
        }
    ...
    fn parse_string_literal(&mut self) -> Result<Term> {
        ...
        if self.input.get(self.pos..self.pos + 2) == Some("^^") {
            ...
            if self.peek_char() == Some('<') {
                ...
                if datatype == "http://loka.dev/f32vec" {
...
mod tests {
    ...
    fn parse_vector_similar_with_threshold() {
        let q = parse(
            r#"SELECT ?doc WHERE { VECTOR_SIMILAR(?doc :hasEmbedding "0.1 0.2 0.3"^^<http://loka.dev/f32vec>, 0.85) }"#,
        )
        .unwrap();
        assert_eq!(q.patterns.len(), 1);
        match &q.patterns[0] {
    ...
    fn parse_vector_similar_with_ef_hint() {
        let q = parse(
            r#"SELECT ?doc WHERE { VECTOR_SIMILAR(?doc :hasEmbedding "0.1 0.2"^^<http://loka.dev/f32vec>, 0.9, ef:=200) }"#,
        )
        .unwrap();
        match &q.patterns[0] {
            Pattern::VectorSimilar {
    ...
    fn parse_vector_similar_with_k_hint() {
        let q = parse(
            r#"SELECT ?doc WHERE { VECTOR_SIMILAR(?doc :hasEmbedding "0.1 0.2"^^<http://loka.dev/f32vec>, 0.9, k:=10) }"#,
        )
        .unwrap();
        match &q.patterns[0] {
            Pattern::VectorSimilar {
    ...
    fn parse_cosine_search() {
        let q = parse(
            r#"SELECT ?doc WHERE { COSINE_SEARCH(?doc :hasEmbedding "0.1 0.2 0.3"^^<http://loka.dev/f32vec>, 0.85) }"#,
        )
        .unwrap();
        match &q.patterns[0] {
    ...
    fn parse_euclid_search() {
        let q = parse(
            r#"SELECT ?doc WHERE { EUCLID_SEARCH(?doc :hasEmbedding "0.1 0.2"^^<http://loka.dev/f32vec>, 0.9) }"#,
        )
        .unwrap();
        match &q.patterns[0] {
    ...
    fn parse_dotproduct_search() {
        let q = parse(
            r#"SELECT ?doc WHERE { DOTPRODUCT_SEARCH(?doc :hasEmbedding "0.5 0.5"^^<http://loka.dev/f32vec>, 0.7) }"#,
        )
        .unwrap();
        match &q.patterns[0] {
            Pattern::MetricSearch { metric, .. } => {
    ...
    fn parse_cosine_search_with_hints() {
        let q = parse(
            r#"SELECT ?doc WHERE { COSINE_SEARCH(?doc :hasEmbedding "0.1 0.2"^^<http://loka.dev/f32vec>, 0.9, ef:=200, k:=10) }"#,
        )
        .unwrap();
        match &q.patterns[0] {
            Pattern::MetricSearch {
    ...
    fn parse_order_by_vector_score() {
        let q = parse(
            r#"SELECT ?doc WHERE { ?doc ?p ?o } ORDER BY DESC(VECTOR_SCORE(?doc :hasEmbedding "0.1 0.2 0.3"^^<http://loka.dev/f32vec>))"#,
        )
        .unwrap();
        assert_eq!(q.order_by.len(), 1);
        assert!(q.order_by[0].descending);
    ...
    fn parse_vector_literal_in_triple() {
        let q = parse(
            r#"SELECT ?s WHERE { ?s :hasEmbedding "0.5 -0.3 1.0"^^<http://loka.dev/f32vec> }"#,
        )
        .unwrap();
        if let Pattern::Triple { object, .. } = &q.patterns[0] {
