...
use loka_core::{DatabaseConfig, TermDictionary, TermId, Triple, TripleStore};
use loka_hnsw::{DistanceMetric, VectorPredicateConfig, VectorRegistry};
use loka_sparql::{execute_with_config, execute_with_vectors, parse};
...
fn star_graph(leaves: usize, categories: usize) -> (TripleStore, TermDictionary) {
    ...
    for i in 0..leaves {
        ...
        store
            ...
            .unwrap();
        let val = loka_core::inline_integer(i as i64).unwrap();
...
fn hnsw_edge_traversal_50_vectors() {
    ...
    for i in 0..50u64 {
        ...
        let vec_id = dict.intern(&format!("\"vec_{}\"^^<http://loka.dev/f32vec>", i));
...
fn vector_search_then_3_hop_traversal() {
    ...
    for i in 0..20u64 {
        ...
        let vec_id = dict.intern(&format!("\"vec_e{}\"^^<http://loka.dev/f32vec>", i));
    ...
    let q = parse(
        "PREFIX ex: <http://example.org/> \
         ...
         VECTOR_SIMILAR(?start ex:hasEmbedding \"1.0 0.0 0.0 0.0\"^^<http://loka.dev/f32vec>, 0.5) \
