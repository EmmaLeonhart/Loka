//! Integration tests for loka-sparql: full query pipeline with vector support.
//!
...
use loka_sparql::{execute_with_config, execute_with_vectors, optimize, parse};
...
fn academic_graph() -> (TripleStore, TermDictionary, VectorRegistry) {
    ...
    let y2017 = loka_core::inline_integer(2017).unwrap();
    let y2018 = loka_core::inline_integer(2018).unwrap();
    let y2020 = loka_core::inline_integer(2020).unwrap();
    let y2019 = loka_core::inline_integer(2019).unwrap();
    let y2021 = loka_core::inline_integer(2021).unwrap();
    ...
    let v1 = dict.intern("\"vec_p1\"^^<http://loka.dev/f32vec>");
    let v2 = dict.intern("\"vec_p2\"^^<http://loka.dev/f32vec>");
    let v3 = dict.intern("\"vec_p3\"^^<http://loka.dev/f32vec>");
    let v4 = dict.intern("\"vec_p4\"^^<http://loka.dev/f32vec>");
    let v5 = dict.intern("\"vec_p5\"^^<http://loka.dev/f32vec>");
...
fn vector_similar_finds_similar_papers() {
    ...
    let q = parse(
        "PREFIX ex: <http://example.org/> \
         SELECT ?paper WHERE { \
         VECTOR_SIMILAR(?paper ex:hasEmbedding \"0.9 0.1 0.0 0.0\"^^<http://loka.dev/f32vec>, 0.7) \
         }",
    )
    .unwrap();
...
fn vector_similar_with_high_threshold() {
    ...
    let q = parse(
        "PREFIX ex: <http://example.org/> \
         SELECT ?paper WHERE { \
         VECTOR_SIMILAR(?paper ex:hasEmbedding \"0.0 0.0 0.95 0.05\"^^<http://loka.dev/f32vec>, 0.9) \
         }",
    )
    .unwrap();
...
fn vector_similar_with_type_constraint() {
    ...
    let mut q = parse(
        "PREFIX ex: <http://example.org/> \
         ...
         VECTOR_SIMILAR(?paper ex:hasEmbedding \"0.0 0.0 0.9 0.1\"^^<http://loka.dev/f32vec>, 0.7) \
         }",
    )
    .unwrap();
...
fn vector_similar_scores_are_populated() {
    ...
    let q = parse(
        "PREFIX ex: <http://example.org/> \
         SELECT ?paper WHERE { \
         VECTOR_SIMILAR(?paper ex:hasEmbedding \"0.9 0.1 0.0 0.0\"^^<http://loka.dev/f32vec>, 0.5) \
         }",
    )
    .unwrap();
...
fn order_by_ascending() {
    ...
    let years: Vec<i64> = result
        .rows
        .iter()
        .map(|r| loka_core::decode_inline_integer(*r.get("year").unwrap()).unwrap())
        .collect();
...
fn vector_similar_no_matches() {
    ...
    let q = parse(
        "PREFIX ex: <http://example.org/> \
         SELECT ?paper WHERE { \
         VECTOR_SIMILAR(?paper ex:hasEmbedding \"0.5 0.5 0.5 0.5\"^^<http://loka.dev/f32vec>, 0.99) \
         }",
    )
    .unwrap();
...
fn vector_similar_with_ef_hint() {
    ...
    let q = parse(
        "PREFIX ex: <http://example.org/> \
         SELECT ?paper WHERE { \
         VECTOR_SIMILAR(?paper ex:hasEmbedding \"0.9 0.1 0.0 0.0\"^^<http://loka.dev/f32vec>, 0.7, ef:=50) \
         }",
    )
    .unwrap();
...
fn vector_similar_with_k_hint() {
    ...
    let q = parse(
        "PREFIX ex: <http://example.org/> \
         SELECT ?paper WHERE { \
         VECTOR_SIMILAR(?paper ex:hasEmbedding \"0.9 0.1 0.0 0.0\"^^<http://loka.dev/f32vec>, 0.0, k:=2) \
         }",
    )
    .unwrap();
...
fn execute_without_vectors_backward_compat() {
    ...
    let result = loka_sparql::execute(&q, &store, &dict).unwrap();
    assert_eq!(result.rows.len(), 5);
}
...
fn planner_puts_vector_first_for_unbound() {
    ...
    let mut q = parse(
        "PREFIX ex: <http://example.org/> \
         ...
         ?doc ex:title ?title . \
         ?thing ?pred ?obj . \
         VECTOR_SIMILAR(?doc ex:hasEmbedding \"0.9 0.1 0.0 0.0\"^^<http://loka.dev/f32vec>, 0.7) \
         }",
    )
    .unwrap();
    ...
    assert!(
        matches!(
            q.patterns[0],
            loka_sparql::parser::Pattern::VectorSimilar { .. }
        ),
...
fn rdf_star_edge_with_vector() {
    ...
    let edge_id = loka_core::quoted_triple_id(alice, knows, bob);
    let conf_val = loka_core::inline_integer(95).unwrap();
    store
    ...
    let result = execute_with_vectors(&q, &store, &dict, &vectors).unwrap();
    assert_eq!(result.rows.len(), 1);
    let conf = loka_core::decode_inline_integer(*result.rows[0].get("conf").unwrap()).unwrap();
    assert_eq!(conf, 95);
}
...
fn rdf_star_wildcard_subject_bound() {
    ...
    // Annotate edges
    let edge1 = loka_core::quoted_triple_id(alice, knows, bob);
    let conf_val = loka_core::inline_integer(90).unwrap();
    store
        .insert(Triple::new(edge1, confidence, conf_val))
        .unwrap();

    let edge2 = loka_core::quoted_triple_id(alice, likes, carol);
    let conf_val2 = loka_core::inline_integer(80).unwrap();
    store
...
fn rdf_star_fully_unbound() {
    ...
    let edge_id = loka_core::quoted_triple_id(alice, knows, bob);
    let conf_val = loka_core::inline_integer(95).unwrap();
    let page_val = loka_core::inline_integer(42).unwrap();
    store
        .insert(Triple::new(edge_id, confidence, conf_val))
        .unwrap();
    store
...
fn virtual_hnsw_edges_queryable_via_sparql() {
    ...
    let _neighbor_id = dict.intern(loka_hnsw::HNSW_NEIGHBOR_IRI);
    ...
    let vec1_id = dict.intern("\"vec_doc1\"^^<http://loka.dev/f32vec>");
    let vec2_id = dict.intern("\"vec_doc2\"^^<http://loka.dev/f32vec>");
    let vec3_id = dict.intern("\"vec_doc3\"^^<http://loka.dev/f32vec>");
    ...
    let q = parse(
        "SELECT ?source ?target WHERE { \
         ?source <http://loka.dev/hnswNeighbor> ?target \
         }",
    )
    .unwrap();
...
fn virtual_hnsw_edges_with_bound_source() {
    ...
    let vec1_id = dict.intern("\"vec1\"^^<http://loka.dev/f32vec>");
    let vec2_id = dict.intern("\"vec2\"^^<http://loka.dev/f32vec>");
    let vec3_id = dict.intern("\"vec3\"^^<http://loka.dev/f32vec>");
