        ...
        b.iter(|| {
            let q = parse(black_box(
                "PREFIX ex: <http://example.org/> \
                 ...
                 \"0.1 0.2 0.3 0.4\"^^<http://loka.dev/f32vec>, 0.8) \
                 ?doc a ex:Document \
                 } LIMIT 10",
            ))
            .unwrap();
...
/// then filter by vector similarity. This is the core Loka use case.
fn bench_graph_then_vector(c: &mut Criterion) {
    ...
    for &n in &[200, 500] {
        ...
        for i in 0..n {
            ...
            let vec_id = dict.intern(&format!("\"pvec_{}\"^^<http://loka.dev/f32vec>", i));
            store
...
fn bench_filter(c: &mut Criterion) {
    ...
    for i in 0..2_000 {
        ...
        store.insert(Triple::new(s, name, name_val)).unwrap();
        let score_val = loka_core::inline_integer(i as i64 % 100).unwrap();
